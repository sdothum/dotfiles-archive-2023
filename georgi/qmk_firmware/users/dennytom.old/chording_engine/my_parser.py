...
def parse_strings_for_chords(data, strings):
    keyboard_part_1 = "\n\n"
...
def write_buffer(output_buffer, output_filepath):
...
def generate_buffer(data):
    ...
    [keyboard_part_2, strings] = parse_chords(data)
    keyboard_part_1 = parse_strings_for_chords(data, strings)
    
    f = open("engine.part.1", "r")
    engine_part_1 = f.read()
    f.close()
    f = open("engine.part.2", "r")
    engine_part_2 = f.read() + "\n"
    f.close()
    f = open("engine.part.3", "r")
    engine_part_3 = f.read()
    f.close()
...
def main(input_filepath, output_filepath):
    with open(input_filepath, "r") as read_file:
        ...
        output_buffer = generate_buffer(data)

        write_buffer(output_buffer, output_filepath)
