...
{
  tap_lt(state, KC_ENT, TRIPLE, _RSYMBOL, tap_rule, 0);  // triple tap -> double enter + shift, down -> enter...
}
...
void equal(qk_tap_dance_state_t *state, void *user_data)
{
  ...
  else { state->pressed ? register_code(KC_EQL) : double_max(state->count, NOSHIFT, KC_EQL); }
  reset_tap_dance(state);
}
