...
static struct key_event {
  ...
  uint8_t  side;
  uint16_t timer;
} e[10];                         // mapped as key col 1 2 3 4 5 <- left, right -> 6 7 8 9 10, index 0 -> no col

static uint8_t rolling_col = 0;  // 0 -> no active *rolling* keycode
...
void clear_layers(void)
{
  ...
  key_timer  = 0;
  tt_keycode = 0;
  clear_e(0);  // no active rolling keycode, see mod_roll()
}
