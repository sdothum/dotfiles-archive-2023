...
add_binds("normal", {
    ...
    buf("^gn$", "Open nzbget.",
        function (w) w:new_tab("http://localhost:6789/nzbget:maaike/") end),
    ...
    buf("^gR$", "Open tiny tiny rss preferences.",
        function (w) w:new_tab("http://luna/tt-rss/prefs.php") end),

    buf("^gr$", "Open tiny tiny rss.",
        function (w) w:new_tab("http://luna/tt-rss/") end),
    ...
    buf("^gT$", "Open tiny tiny rss preferences.",
        function (w) w:new_tab("http://localhost/tt-rss/prefs.php") end),

    buf("^gt$", "Open tiny tiny rss.",
        function (w) w:new_tab("http://localhost/tt-rss/") end),
    ...
    key({}, "F8", "Open localhost://thedarnedestthing/.",
        function (w) w:new_tab("http://thedarnedestthing/") end),

    key({"Shift"}, "F8", "Open localhost://thedarnedestthing/resources.",
        function (w) w:new_tab("http://thedarnedestthing/resources") end),

    key({"Control"}, "F8", "Admin localhost://thedarnedestthing/.",
        function (w, c) w:enter_cmd(":open http://thedarnedestthing/admin/do%20:") end),

    key({}, "F7", "List thedarnedestthing draft articles.",
        function (w) w:new_tab("http://thedarnedestthing/admin/do%20:draft") end),

    key({"Shift"}, "F7", "List thedarnedestthing orphan articles.",
        function (w) w:new_tab("http://thedarnedestthing/admin/do%20:orphan") end),
    ...
    key({"Control"}, "F4", "My lastpass login.",
        function (w)
            w.view:eval_js("function(){/*Click_This_Button_To_AutoLogin___Copyright_LastPass_all_rights_reserved*/_LASTPASS_INC=function(u,s){if(u.match(/_LASTPASS_RAND/)){alert('Cancelling_request_may_contain_randkey');return;}s=document.createElement('script');s.setAttribute('type','text/javascript');s.setAttribute('charset','UTF-8');s.setAttribute('src',u);if(typeof(window.attachEvent)!='undefined')document.body.appendChild(s);else{if(document.getElementsByTagName('head').length>0){document.getElementsByTagName('head').item(0).appendChild(s);}else{document.getElementsByTagName('body').item(0).appendChild(s);}}};_LASTPASS_RAND='60dae8607d4c1b6b8cd6716b9631e65264d25c60b0a05a740a8635727b07267b';_LASTPASS_INC('https://lastpass.com/bml.php'+String.fromCharCode(63)+'v=0&a=1&r='+Math.random()+'&h=b37fedb1028bba2403f0fd3da4ad3849e4f2b9228b27efd52c01beaa71d0b05c&u='+escape(document.location.href));})()")
        end),

    key({}, "F4", "My readability script.",
        function (w)
            -- http://readable.tastefulwords.com/
            w.view:eval_js("(function(){_readableOptions={'text_font':'Helvetica Neue, Arial, DejaVu Sans, Verdana, Geneva, sans-serif','text_font_monospace':'Inconsolata','text_font_header':'Helvetica, quote(Helvetica Neuve), Arial, Tahoma, sans-serif','text_size':'14px','text_line_height':'1.7','box_width':'37em','color_text':'#CCCCCC','color_background':'#343A3A','color_links':'#99CCFF','text_align':'normal','base':'blueprint','custom_css':''};if(document.getElementsByTagName('body').length>0);else{return;}if(window.$readable){if(window.$readable.bookmarkletTimer){return;}}else{window.$readable={};}window.$readable.bookmarkletTimer=true;window.$readable.options=_readableOptions;if(window.$readable.bookmarkletClicked){window.$readable.bookmarkletClicked();return;}_readableScript=document.createElement('script');_readableScript.setAttribute('src','http://readable-static.tastefulwords.com/target.js?rand='+encodeURIComponent(Math.random()));document.getElementsByTagName('body')[0].appendChild(_readableScript);})()")
            -- w:set_mode("passthrough")
            w:set_mode("normal")
        end),
...
webview.init_funcs.magnet_hook = function (view, w)
    view:add_signal("navigation-request", function (v, uri)
        if string.match(uri, "^magnet:") then
            local cmd = string.format("%s %q", "tixati", uri)
            luakit.spawn(cmd)
            return false
        end
    end)
...
webview.methods.browse_history_dmenu = function( view, w )
    ...
    local filter = '"ERROR:|localhost|luakit:|Page not foundch"'
    local dmenu = "$(dlist) -p 'Luakit History:'"
...
webview.methods.browse_bookmarks_dmenu = function( view, w )
    ...
    local dmenu = "$(dlist) -p 'Luakit Bookmark:'"
