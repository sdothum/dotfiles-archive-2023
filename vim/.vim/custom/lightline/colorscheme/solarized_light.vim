...
" condition vim-colors-solarized_light uses to determine which set of colors
" to use.
let s:solarized_light_termcolors = get(g:, 'solarized_light_termcolors', 256)
if s:solarized_light_termcolors != 256 && &t_Co >= 16
  let s:cuiindex = 0
elseif s:solarized_light_termcolors == 256
  let s:cuiindex = 1
else
  let s:cuiindex = 2
endif
...
let s:p.visual.left = [ [ s:base03, s:magenta ], [ s:base03, s:base1 ], [ s:base03, s:base00 ], [ s:green, s:base02 ] ]
