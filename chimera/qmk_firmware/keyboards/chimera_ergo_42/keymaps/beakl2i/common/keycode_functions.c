...
void mod_roll(keyrecord_t *record, uint8_t side, uint8_t shift, uint16_t modifier, uint16_t keycode, uint8_t column)
{
  if (record->event.pressed) {
    if (keycode == e[column].keycode) { register_code(keycode); e[column].keycode = 0; return; }
    ...
    if (!e[column].keycode) { unregister_code(keycode); e[column].key_timer = 0; return; }
    ...
    else { e[column].keycode = 0; }                          // disable auto-repeat
