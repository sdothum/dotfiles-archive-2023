...
check_sent    = exists("/home/shum/.sup/.check_sent")
...
inbox   = account['sdothum/[Gmail]/All Mail']:select_all()
...
if check_sent then
  os.remove("/home/shum/.sup/.check_sent")
end
...
print "\n::::  providers  ::::\n"
from    = '(1channel.ch|(?i)2checkout.com|2co.com|astraweb.com|docdatapayments.com|fatcow|GoDaddy|gogo6|gogoNET|newztown.co.za|nMatrix|no-ip.com|nzbgeek.info|NZBGrabit|(NZBM|nzbm)atrix|nzbs2go.com|PayFast|(?i)servarica|sunnyusenet.com|tweaknews.eu|usenet-crawler.com|xennews.com|Zenfolio)'
results = inbox:match_from(from) + inbox:match_to(from)
if check_sent then
  results = results + sent:match_to(from)
end
...
print "\n::::  services  ::::\n"
from    = '(Canada Post|canadapost.postescanada.ca|cooperators.ca|enercare.ca|fedex.com|HUNTCLUBVW.COM|ic.gc.ca|cards@jacquielawson.com|huntclubvw.com|(?i)keepcalling.(ca|com)|MobileMe|ndp.ca|ottawa.ca|Pretoria Pet Hospital|parl.gc.ca|primustel.ca|Skype|usps.com)'
results = inbox:match_from(from) + inbox:match_to(from)
if check_sent then
...
print "\n::::  community  ::::\n"
from    = '(workshops@anatomyoftrauma.com|Abbey Zat|Abigail Szathmary|Adam Duncan|Barbara Clubb|bbielous@comcast.net|Calvin Dale|(?i)cameron.macinnes|Cheryl Brost|Chris Hayes|Dara Leonard|Darlene Pearson|David Somerville|Donna Prokopetz|Donna Somerville|donnasom@nbnet.nb.ca|dorothy cameron|DOUGLAS.MACINNES|elephantabbey@yahoo.ca|erowan@bell.net|(?i)evelyn.*ennor|Gaela Morrison|Gord Gamble|granskou@magma.ca|herwig schoen|ibuchanan999@gmail.com|Jan Erik Ask|Jan Falls|JANICE FALLS|Jason Blight|Jean Ogilvie|Jean-Nil Boulanger|JO ANN SHILLINGTON|Joan Gamble|jonathan@gallivanmedia.com|Joy Greenleese|Joy Schwartz|Judy Jarvis|Kenneth Prokopetz|Kjell|Lee Manu|Louise Dubien|M Joan Freeman|MacInnes, Cameron|Mangal Waghmare|Marc Dupuis|Mary Ann Eddowes|michi@live.ca|Nicole Lehmann|normantech@gmail.com|(Red|Sonja) (Sonja|Red)|Richard Norman|Robert Boylin|Roger and Gayle|Roger Leblanc|Sayoko Konno|Seema|Shahid Parvez Khan|Siyam Rafique|Stephanie Cairns|Stephanie Meade|Swami Sivananda|Tahra Bielous|Tom Tomlinson|Vicki Tansey)'
results = inbox:match_from(from) + inbox:match_to(from)
if check_sent then
  results = results + sent:match_to(from)
end
...
print "\n::::  family  ::::\n"
from    = '((?i)ank.wensink|Anne Patterson|Becci Hayes|Chan Ron|Christopher Patterson|Emily McInnes|Gil Hartley|gilandsuehartley@gmail.com|Hum.*Hartley|Janice.Hum_patterson@calgary.ca|Jeroen Hum|jeroen.hum|Johanna Wensink|Kendra Hartley|Leah Eustace|(?i)leonie.hum|ln522848@dal.ca|Maaike Hum|mbpatter@telusplanet.net|Melissa Moffat|Nick & Grace|Nicholas Hartley|Pat & Bill Moffat|Pat Moffat|Patricia Hayes|sairinhayes@rogers.com|Taila Hartley)'
...
print "\n::::  personal  ::::\n"
from    = '(acuityscheduling.com|cdalesmith@calvindale.com|Capital One|cooperators.ca|drfarid.com|Energy Thrive|energythrive|Geoff Duncan|heidi_mnpc@rogers.com|L Duncan|Laura Barber|Mark Smyth|Michael Pallett|Northcote CA|Pallett, Michael|Paul Menard|Sandra Zlobina|Sandy Turcotte|Smyth, Mark D|Valerie Nixon|Victoria Manning|Victoria Peters|(?i)w.*f.*tyler)'
results = inbox:match_from(from) + inbox:match_to(from)
if check_sent then
  results = results + sent:match_to(from)
end
...
results = account['sdothum/[Gmail]/All Mail']:select_all()
